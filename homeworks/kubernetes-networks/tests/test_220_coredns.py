...
def preload_manifests(path: str, selector: Dict[str, str]) -> List[models.V1Service]:
    """
...
MANIFESTS = preload_manifests(
    path="./kubernetes-networks", selector={"k8s-app": "kube-dns"}
)
...
def get_services_by_proto(proto_string: str) -> List[models.V1Service]:
    return [obj for obj in MANIFESTS if obj.spec.ports[0].protocol == proto_string]


@pytest.fixture(scope="module")
def coredns_tcp(kube_module: kubetest.client.TestClient) -> kubetest.objects.Service:
...
@pytest.fixture(scope="module")
def coredns_udp(kube_module: kubetest.client.TestClient) -> kubetest.objects.Service:
...
def test_coredns_shared_ip(
    coredns_tcp: kubetest.objects.Service, coredns_udp: kubetest.objects.Service
) -> None:
...
def test_coredns_tcp(
    coredns_tcp: kubetest.objects.Service, test_container: testinfra.host.Host
) -> None:
    assert isinstance(
    ...
    assert (
        coredns_tcp.obj.status.load_balancer != {}
    ...
    tcp: str = test_container.check_output(
...
def test_coredns_udp(
    coredns_udp: kubetest.objects.Service, test_container: testinfra.host.Host
) -> None:
    ...
    assert (
        coredns_udp.obj.status.load_balancer.ingress != {}
    ), "LoadBalancer ingress status is not set. Is MetalLB available?"
    ...
    udp: str = test_container.check_output(
